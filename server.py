...
def init_db():
    ...
    try:
        ...
        conn.rollback()
    # One-time bulk import of maintenance parts from Streamline (Kristin's
    # request — 453 items across 17 categories, SBR Loaners excluded since
    # those are tracked separately in the Loaners feature). Uses the existing
    # Maintenance Supply screen/table (supply_items) — nothing new was built,
    # this just populates what was already there. Idempotent on name (unique
    # constraint) — safe to run on every startup, never overwrites a quantity
    # someone has since adjusted by hand. Low-stock threshold starts at 0 for
    # every item (alert only when truly out) rather than the usual default of
    # 5, since a flat 5 would have immediately flagged the majority of these
    # 453 items as low stock on day one — individual thresholds can be raised
    # per item later via the existing edit button once real reorder points
    # are known.
    try:
        maintenance_parts_seed = [
        ('35+5 Capacitor', 'AC Repair', 3),
        ('40+5 Capacitor', 'AC Repair', 0),
        ('45+5 Capacitor', 'AC Repair', 3),
        ('50+5 Capacitor', 'AC Repair', 1),
        ('AC CORD', 'AC Repair', 1),
        ('AC drain cleaner bottle', 'AC Repair', 2),
        ('Blue/white coil brush', 'AC Repair', 2),
        ('Double Pole Contactor', 'AC Repair', 1),
        ('Flow Tablets (Single)', 'AC Repair', 450),
        ('Honeywell thermostat', 'AC Repair', 2),
        ('Single Pole Contactor', 'AC Repair', 5),
        ('Swoosh Shots (Single)', 'AC Repair', 37),
        ('Thermostat Wire', 'AC Repair', 1),
        ('Transformer', 'AC Repair', 1),
        ('½" CPVC cap', 'AC Repair', 1),
        ('½" PVC caps', 'AC Repair', 0),
        ('¾" PVC 90', 'AC Repair', 2),
        ('¾" PVC Couplers', 'AC Repair', 11),
        ('¾" PVC trap', 'AC Repair', 1),
        ('2032', 'Batteries', 51),
        ('9 Volt', 'Batteries', 107),
        ('A23', 'Batteries', 50),
        ('AA', 'Batteries', 386),
        ('AAA', 'Batteries', 206),
        ('C Batteries', 'Batteries', 0),
        ('D', 'Batteries', 68),
        ('LR44', 'Batteries', 21),
        ('Watch Battery', 'Batteries', 0),
        ('2.5 Gal 30 Sec Cleaner', 'Cleaning Products', 1),
        ('5 Gal Outdoor Cleaner Concentrate', 'Cleaning Products', 1),
        ('Active Mold Stain Remover', 'Cleaning Products', 4),
        ('ArmourAll Tire Foam Cloth', 'Cleaning Products', 1),
        ('Microfiber 12 pack', 'Cleaning Products', 1),
        ('Dryer Vent Clean Jr.', 'Cleaning Products', 1),
        ('Gallon of purple power', 'Cleaning Products', 2),
        ('Gloves', 'Cleaning Products', 1),
        ('Ice Machine Cleaner', 'Cleaning Products', 7),
        ('Laundry Detergent', 'Cleaning Products', 2),
        ('Meguiar\'s Gold Class High Gloss Premium Quik Detailer', 'Cleaning Products', 8),
        ('Scotch Brite Pad', 'Cleaning Products', 5),
        ('Stainless Steel Cleaner', 'Cleaning Products', 2),
        ('Windex Refill', 'Cleaning Products', 4),
        ('Door Handle', 'Door Parts & Point Central', 1),
        ('Extender - Point Central Range extender', 'Door Parts & Point Central', 2),
        ('Handle - lever passage', 'Door Parts & Point Central', 2),
        ('Handle - Round keyed Privacy', 'Door Parts & Point Central', 1),
        ('Handle - round passage', 'Door Parts & Point Central', 1),
        ('Hub - Point Central', 'Door Parts & Point Central', 12),
        ('Hub Brackets', 'Door Parts & Point Central', 14),
        ('Lock - Yale Pro', 'Door Parts & Point Central', 3),
        ('Point Central Power Cord', 'Door Parts & Point Central', 3),
        ('Privacy Lever', 'Door Parts & Point Central', 1),
        ('RD POCKET DOOR LOCK', 'Door Parts & Point Central', 4),
        ('Smart Thermostat', 'Door Parts & Point Central', 4),
        ('Tumbler', 'Door Parts & Point Central', 1),
        ('Battery Relay', 'Electric Cart Repair', 1),
        ('Cart - AC port plug', 'Electric Cart Repair', 0),
        ('Cart Reflectors', 'Electric Cart Repair', 2),
        ('Charging cord (SWC Revo)', 'Electric Cart Repair', 3),
        ('Cup Holder', 'Electric Cart Repair', 1),
        ('EVO Charger', 'Electric Cart Repair', 1),
        ('Headlight (H11 Cart)', 'Electric Cart Repair', 1),
        ('ICON Aetric (Orange Charge Cord)', 'Electric Cart Repair', 2),
        ('Light Bulbs - Headlight', 'Electric Cart Repair', 2),
        ('Light Bulbs - Turn Signal', 'Electric Cart Repair', 1),
        ('Seatbelt', 'Electric Cart Repair', 0),
        ('Silverstar Headlight ( 2- pack)', 'Electric Cart Repair', 1),
        ('Tire Plug Kit', 'Electric Cart Repair', 10),
        ('Turn Signal Relay', 'Electric Cart Repair', 1),
        ('Waterproof Key Fob Protectors', 'Electric Cart Repair', 10),
        ('12" Polished Pewter Down Rod', 'Electrical', 2),
        ('3 way Deco (White)', 'Electrical', 5),
        ('3 Way Lamp Turn Knob Housing', 'Electrical', 1),
        ('Breaker (20amp Double Pole)', 'Electrical', 3),
        ('Bulb Adapter', 'Electrical', 0),
        ('Ceiling Fan Wall Control Slider Switch', 'Electrical', 3),
        ('Coax Wall Plates', 'Electrical', 3),
        ('Dimmer Replacement', 'Electrical', 3),
        ('Dimmer Replacement Knob', 'Electrical', 7),
        ('Dimmer Switch (Lutron LECL-153P- WH-3)', 'Electrical', 2),
        ('Dimmer Switch (Single Pole Toggle)', 'Electrical', 2),
        ('Dual Gang Electrical Outlet Covers', 'Electrical', 5),
        ('Fan Capacitor', 'Electrical', 2),
        ('GFI 15a', 'Electrical', 8),
        ('GFI 20a', 'Electrical', 8),
        ('Lamp Harp', 'Electrical', 1),
        ('Lamp Pull Chain Socket', 'Electrical', 1),
        ('Light Socket Shade Ring 20 PCS', 'Electrical', 2),
        ('Light Sockets (G4 base)', 'Electrical', 6),
        ('Outlet Covers (Duplex)', 'Electrical', 25),
        ('Pull Chain Switch', 'Electrical', 0),
        ('Round Ceiling Box', 'Electrical', 2),
        ('Single Pole Toggle', 'Electrical', 13),
        ('Snap on plug', 'Electrical', 1),
        ('Standard Outlet', 'Electrical', 27),
        ('Switch (Single Pole White Decorator)', 'Electrical', 1),
        ('Switch Cover ( Toggle White)', 'Electrical', 24),
        ('Switch Cover (Double)', 'Electrical', 2),
        ('Toggle Switch 3 Way Non Dimmer', 'Electrical', 5),
        ('Wall Cover (Single-White Blank)', 'Electrical', 7),
        ('Wire Nuts - (22-6 AWG)', 'Electrical', 64),
        ('Wire Twist', 'Electrical', 1),
        ('Firewood (regular)', 'Firewood', 0),
        ('Adjustable heat shield (Charbroil)', 'Grill/Appliance Parts', 1),
        ('Charboil Grill Igniter', 'Grill/Appliance Parts', 3),
        ('Charboril Crossover Tube Kit', 'Grill/Appliance Parts', 1),
        ('Charbroil Replacement Gas Valve Clamp', 'Grill/Appliance Parts', 1),
        ('CSSL54-60 CSS GAS CONN', 'Grill/Appliance Parts', 1),
        ('Dishwasher Spray Arm', 'Grill/Appliance Parts', 1),
        ('Dishwasher Wheels', 'Grill/Appliance Parts', 42),
        ('Drip pan liner (Charbroil)', 'Grill/Appliance Parts', 0),
        ('Drip pan liner (Weber Grill)', 'Grill/Appliance Parts', 10),
        ('Dryer Power Cord', 'Grill/Appliance Parts', 1),
        ('Garbage Disposal - 1/2', 'Grill/Appliance Parts', 0),
        ('Garbage Disposal - 3/4', 'Grill/Appliance Parts', 0),
        ('Granite Countertop Dishwasher Installation Kit', 'Grill/Appliance Parts', 1),
        ('Grill Brush Spiral', 'Grill/Appliance Parts', 14),
        ('Grill Utensil Sets', 'Grill/Appliance Parts', 4),
        ('Ice Tray', 'Grill/Appliance Parts', 2),
        ('Igniter (Weber grill switch)', 'Grill/Appliance Parts', 0),
        ('Igniter Kit', 'Grill/Appliance Parts', 4),
        ('Ignitor (Electric Kit)', 'Grill/Appliance Parts', 3),
        ('Large Grill Cover', 'Grill/Appliance Parts', 0),
        ('Lower dishrack roller and axle kit', 'Grill/Appliance Parts', 1),
        ('Medium Grill Cover', 'Grill/Appliance Parts', 2),
        ('Microwave plate', 'Grill/Appliance Parts', 2),
        ('Oven Thermometer', 'Grill/Appliance Parts', 1),
        ('Propane Tank Exchange', 'Grill/Appliance Parts', 14),
        ('Small Grill Cover', 'Grill/Appliance Parts', 2),
        ('Stainless Steel Heat Tent', 'Grill/Appliance Parts', 2),
        ('Uniflasy Universal Adjustable Crossover Channel Tubes Replacemen', 'Grill/Appliance Parts', 1),
        ('Warming Rack', 'Grill/Appliance Parts', 1),
        ('Weber Grill Cover', 'Grill/Appliance Parts', 0),
        ('XL Grill Cover', 'Grill/Appliance Parts', 0),
        ('2x4x8 Pressure Treated', 'Hardware', 1),
        ('4.5 Inch Felt Pads', 'Hardware', 5),
        ('5/4 Deck Board', 'Hardware', 4),
        ('APPLIANCE EPOXY WHT', 'Hardware', 1),
        ('Assorted set screw kit', 'Hardware', 1),
        ('Ball Chain Roll', 'Hardware', 1),
        ('Blend Fil Pencil', 'Hardware', 2),
        ('BONDO: 1-QT BODY WOOD FILLER', 'Hardware', 2),
        ('Bottom Door Seal', 'Hardware', 2),
        ('Caulk Grout - Biscuit', 'Hardware', 1),
        ('Caulk Tube (White)', 'Hardware', 7),
        ('CEILING BLANK UP PLATE', 'Hardware', 2),
        ('Clear Safety Chain Retainer and Cord', 'Hardware', 60),
        ('Closet Flange Set (Heavy Duty)', 'Hardware', 1),
        ('Concrete Mix', 'Hardware', 0),
        ('Curtain hooks', 'Hardware', 21),
        ('Deck Screws', 'Hardware', 285),
        ('Door Stop -', 'Hardware', 6),
        ('Door Stop Hinge Pin Chrome', 'Hardware', 3),
        ('Doorstop (Chrome spring stops)', 'Hardware', 10),
        ('Doorstop (Rigid chrome)', 'Hardware', 5),
        ('Doorstop tips', 'Hardware', 11),
        ('Dryer vent (Rigid)', 'Hardware', 0),
        ('Drywall Screws - 2"', 'Hardware', 240),
        ('Dummy Doorknob - Round', 'Hardware', 0),
        ('EPOXY WOODWELD TAN .85OZ', 'Hardware', 0),
        ('Exhaust Fan Grate', 'Hardware', 2),
        ('Flat screen spline roll -50 inch', 'Hardware', 0),
        ('Furniture Felt Pads - 3/4', 'Hardware', 60),
        ('Furniture Felt Pads- 1 1/2"', 'Hardware', 40),
        ('Furniture Felt Pads- 1\'\'', 'Hardware', 64),
        ('Furniture Felt Pads- 2"', 'Hardware', 24),
        ('Key Storage Box', 'Hardware', 1),
        ('Lamp Shade Harp', 'Hardware', 0),
        ('Lamp Socket Knob', 'Hardware', 0),
        ('Latch Knob', 'Hardware', 1),
        ('Lock Hasp Charm', 'Hardware', 1),
        ('Machine Round Combo with Nuts', 'Hardware', 100),
        ('Mini paint roller frame', 'Hardware', 1),
        ('Paint Brush 3\'\'', 'Hardware', 0),
        ('Paint Knap - 9\'\'', 'Hardware', 3),
        ('Paint Knap- 18\'\'', 'Hardware', 4),
        ('Painters Tape', 'Hardware', 0),
        ('Penny Tile', 'Hardware', 0),
        ('Pillow Covers', 'Hardware', 3),
        ('Pipe support', 'Hardware', 0),
        ('Pocket Door Lock - Satin Nickel', 'Hardware', 7),
        ('Post Cap 4x4', 'Hardware', 3),
        ('Pull Knobs - Cabinet', 'Hardware', 12),
        ('Recessed cabinet hinge', 'Hardware', 1),
        ('Rug Gripper', 'Hardware', 18),
        ('SCREEN', 'Hardware', 21),
        ('Seat and spring Sets', 'Hardware', 3),
        ('Shower Sweep (Bottom)', 'Hardware', 6),
        ('Shutter Blade Repair', 'Hardware', 13),
        ('Silicon Tube (Pewter)', 'Hardware', 1),
        ('Silicone Tube (Almond)', 'Hardware', 1),
        ('Silicone Tube (Armor)', 'Hardware', 1),
        ('Silicone Tube (Cobbletsone)', 'Hardware', 1),
        ('Silicone Tube (Seasalt)', 'Hardware', 1),
        ('Silicone Tube (Warm Gray)', 'Hardware', 1),
        ('Silicone Tube - Tub and Tile', 'Hardware', 8),
        ('Sliding Door Lock (Solid Brass)', 'Hardware', 1),
        ('Sliding Door Pull (solid Brass)', 'Hardware', 1),
        ('Speaker Grate - 8\'\'', 'Hardware', 2),
        ('Speaker Magnets', 'Hardware', 50),
        ('SS36 Hose Clamp', 'Hardware', 2),
        ('Stainless Gate Hook and Eye - 2"', 'Hardware', 0),
        ('Stainless Gate Hook and Eye - 3"', 'Hardware', 0),
        ('Strong Strap Stretch Latch', 'Hardware', 4),
        ('Tapcon Hex (Concrete)', 'Hardware', 16),
        ('Toggle Bolt - 1/8x2\'\'', 'Hardware', 46),
        ('TOOL EMERGENCY F/PRVCY LOCK CP', 'Hardware', 2),
        ('Tri-Glow Superior Lubricant', 'Hardware', 1),
        ('Trimmer Line WLS-1105', 'Hardware', 1),
        ('Velcro - 15\'', 'Hardware', 1),
        ('Water Weld Epoxy Sticks', 'Hardware', 2),
        ('White Surface Speaker Covers 10"', 'Hardware', 4),
        ('WLM-H95 Trimmer Line', 'Hardware', 1),
        ('Wood Block', 'Hardware', 1),
        ('Wood Filler Pencil', 'Hardware', 3),
        ('Wood Finish (can)', 'Hardware', 2),
        ('Wood Putty (jar)', 'Hardware', 1),
        ('12\'\' T5 LED', 'Light Bulbs', 3),
        ('40 W - Halogen Bulb', 'Light Bulbs', 5),
        ('60-watt Bulb', 'Light Bulbs', 91),
        ('APPLIANCE BULB 25W T8', 'Light Bulbs', 3),
        ('Bulb g8', 'Light Bulbs', 10),
        ('Can Light (5- 6")', 'Light Bulbs', 36),
        ('CFL 14 Watt 4 pin', 'Light Bulbs', 4),
        ('CFL 9 Watt 2 Pin', 'Light Bulbs', 3),
        ('Chandelier - Large Base', 'Light Bulbs', 29),
        ('Chandelier - Small Base', 'Light Bulbs', 131),
        ('Class B 1157', 'Light Bulbs', 3),
        ('Clear Candelabra - G161/2', 'Light Bulbs', 3),
        ('Dimmable BR30', 'Light Bulbs', 3),
        ('E-11', 'Light Bulbs', 15),
        ('Flood Light - PAR30S', 'Light Bulbs', 1),
        ('Fluorescent Bulb - 48\'\'', 'Light Bulbs', 2),
        ('G24Q Bulb', 'Light Bulbs', 1),
        ('G4 Led', 'Light Bulbs', 15),
        ('G40 Base', 'Light Bulbs', 1),
        ('G9', 'Light Bulbs', 34),
        ('GY 6.35', 'Light Bulbs', 8),
        ('Heat Lamp 125W', 'Light Bulbs', 2),
        ('Landscaping Bulbs', 'Light Bulbs', 8),
        ('Mini Candelabra', 'Light Bulbs', 13),
        ('Mini Flood PAR20', 'Light Bulbs', 7),
        ('Mini Flood- PAR16', 'Light Bulbs', 2),
        ('Standard Flood', 'Light Bulbs', 140),
        ('Stove Light - GU10 (twist in)', 'Light Bulbs', 7),
        ('Stove Light - GU5.3(pin)', 'Light Bulbs', 3),
        ('1- 3/8 inch Tub stopper', 'Plumbing', 2),
        ('1/2" CPVC Pipe Strap', 'Plumbing', 5),
        ('3/8" PVC SharkBite shut off valve', 'Plumbing', 2),
        ('Ball rod Kit - 6"', 'Plumbing', 1),
        ('Ball valve (3/4")', 'Plumbing', 1),
        ('Ballcock Fluidmaster', 'Plumbing', 0),
        ('Basket strainer', 'Plumbing', 1),
        ('Bronze Shark Bite Couplings', 'Plumbing', 6),
        ('Cartridge assembly (Kohler)', 'Plumbing', 5),
        ('Drain plunger (Kohler eyebolt)', 'Plumbing', 9),
        ('Drain Stopper (Sink)', 'Plumbing', 8),
        ('Elongated Toilet Seat', 'Plumbing', 5),
        ('Faucet (Moen single handle)', 'Plumbing', 3),
        ('Faucet Spray Head (Kohler)', 'Plumbing', 1),
        ('Faucet Supply line - 20"', 'Plumbing', 10),
        ('Flapper (3" Universal)', 'Plumbing', 10),
        ('Flapper 2in', 'Plumbing', 13),
        ('Fluidmaster Fill Valve', 'Plumbing', 2),
        ('Flush repair kit (Dual)', 'Plumbing', 1),
        ('Flush valve', 'Plumbing', 4),
        ('Flush Valve Seal', 'Plumbing', 11),
        ('Garbage disposal switch', 'Plumbing', 1),
        ('Jumbo Wax Ring', 'Plumbing', 2),
        ('Outdoor Shower Kit', 'Plumbing', 1),
        ('Plumbers\' putty (14oz bottle)', 'Plumbing', 1),
        ('Pop Up Plunger', 'Plumbing', 7),
        ('popup stopper fitsall', 'Plumbing', 2),
        ('PVC Couplings', 'Plumbing', 2),
        ('PVC Elbow Couplings', 'Plumbing', 15),
        ('PXPC34 PINCH CLAMP PEX 3/4', 'Plumbing', 0),
        ('Rain bird droppers 1/4in', 'Plumbing', 2),
        ('Raindrop Dripper - 1 gal', 'Plumbing', 10),
        ('Raindrop Dripper - 2 gal', 'Plumbing', 5),
        ('Rubber Gasket', 'Plumbing', 3),
        ('Shark bite (3/4" x 3/4")', 'Plumbing', 5),
        ('Sink aerator (female)', 'Plumbing', 0),
        ('sink pull rod', 'Plumbing', 2),
        ('Sink stopper (1" rubber)', 'Plumbing', 2),
        ('Slip joint nut', 'Plumbing', 1),
        ('Sprinkler connection - 1/2 rain bird T joint', 'Plumbing', 10),
        ('Sprinkler connection 1/2in rain bird coupling', 'Plumbing', 7),
        ('Sprinkler connection - 1/2in rain bird elbow', 'Plumbing', 10),
        ('Sprinkler Connection - 1/4in Barbed tee', 'Plumbing', 10),
        ('Sprinkler connection - 1/4in rain bird pipe', 'Plumbing', 1),
        ('Sprinkler Connection - Rain Bird 1/4 Elbows', 'Plumbing', 9),
        ('Sprinkler Connection - Rain Bird 1/4 in Couplers', 'Plumbing', 6),
        ('Sprinkler Connection - Rain Bird T - 5/8', 'Plumbing', 1),
        ('Sprinkler Connection - Rain Bird 1/2 Tubing', 'Plumbing', 101),
        ('Sprinkler Connections - Rain Bird Bubblers', 'Plumbing', 3),
        ('Stopper extension (Kohler)', 'Plumbing', 17),
        ('stopper Rubber 1 1/4', 'Plumbing', 1),
        ('Tank Bolts', 'Plumbing', 3),
        ('Tank to Bowl Kit', 'Plumbing', 2),
        ('Toilet Handle', 'Plumbing', 4),
        ('Toilet paper holder bar', 'Plumbing', 8),
        ('Toilet paper holder kit', 'Plumbing', 1),
        ('Toilet Seat Round', 'Plumbing', 4),
        ('Toilet supply (12")', 'Plumbing', 11),
        ('ToTo Toilet Handle', 'Plumbing', 0),
        ('Tub side flange', 'Plumbing', 14),
        ('Washer hose set', 'Plumbing', 1),
        ('Wax ring (standard)', 'Plumbing', 2),
        ('Adjustable Pool Pole', 'Pool Supplies', 0),
        ('Color Pool Wall Light w/ Remote', 'Pool Supplies', 2),
        ('Deluxe Solar Underwater Light', 'Pool Supplies', 3),
        ('Pool Auto Filler', 'Pool Supplies', 1),
        ('pool net', 'Pool Supplies', 8),
        ('Pool skimmer basket', 'Pool Supplies', 13),
        ('Alarm - CO', 'Safety', 0),
        ('Alarm - CO2 (wired/plug in)', 'Safety', 1),
        ('Alarm - smoke (wired)', 'Safety', 1),
        ('CU2 emergency lights', 'Safety', 4),
        ('Fire Extinguisher', 'Safety', 1),
        ('Fire Extinguisher Stickers', 'Safety', 61),
        ('Outside Can Lights', 'Safety', 3),
        ('Pool Door/Gate Alarms', 'Safety', 25),
        ('Pool Gate Safety Latch', 'Safety', 1),
        ('1.25 gal gas can', 'SBR Equipment', 2),
        ('12x15 Drop Cloth', 'SBR Equipment', 6),
        ('2 Gal Poly Sprayer', 'SBR Equipment', 5),
        ('2.5 wet/Dry Vac', 'SBR Equipment', 1),
        ('27in By-Pass Lopper Deluxe', 'SBR Equipment', 1),
        ('4 ft Ladder', 'SBR Equipment', 2),
        ('50 ft Water Fed Pole', 'SBR Equipment', 1),
        ('50\' Pressure Washer Hose', 'SBR Equipment', 2),
        ('6 ft Charger Cord', 'SBR Equipment', 1),
        ('9 Gallon Corded Shop Wet/Dry Vacuum', 'SBR Equipment', 1),
        ('AC Drain Line Tool', 'SBR Equipment', 0),
        ('Adjustable pool pole', 'SBR Equipment', 1),
        ('Air Compressor and Hose', 'SBR Equipment', 3),
        ('Air-Powered Nail Gun (Brad)', 'SBR Equipment', 2),
        ('Animal Trap (medium)', 'SBR Equipment', 1),
        ('Auger Bit 5/8', 'SBR Equipment', 1),
        ('Auger Electric Snake (50ft)', 'SBR Equipment', 1),
        ('Auger Snake (Drain)', 'SBR Equipment', 2),
        ('Bed and Bath Door Pin', 'SBR Equipment', 7),
        ('Blower Handheld (STHL)', 'SBR Equipment', 4),
        ('Bolt Cutter', 'SBR Equipment', 1),
        ('Bottom Feeder Debris Bags', 'SBR Equipment', 3),
        ('Bottom Feeder Swtich Cord', 'SBR Equipment', 1),
        ('Bow Rake', 'SBR Equipment', 1),
        ('Broom', 'SBR Equipment', 5),
        ('Buffer', 'SBR Equipment', 1),
        ('Canopy Tent', 'SBR Equipment', 1),
        ('Caulking gun', 'SBR Equipment', 4),
        ('Cleaner Ext Hose End 64 oz', 'SBR Equipment', 4),
        ('Colorsnap Match Pro', 'SBR Equipment', 1),
        ('Crimp Ring Removal Tool', 'SBR Equipment', 1),
        ('Diversitech MBK 6 Drain Kat', 'SBR Equipment', 1),
        ('Drain Pump Assembly (Ice Machine)', 'SBR Equipment', 1),
        ('Drain Weasal (Sink)', 'SBR Equipment', 7),
        ('Dryer Vent Brush Kit', 'SBR Equipment', 1),
        ('Dryer Vent Ext Kit 4-3ft Rods', 'SBR Equipment', 4),
        ('Drywall Knife', 'SBR Equipment', 1),
        ('Duster Cob Webb', 'SBR Equipment', 3),
        ('DW Recip Saw', 'SBR Equipment', 1),
        ('Electric Palm Sander Pads', 'SBR Equipment', 10),
        ('Extension Cord - 25ft', 'SBR Equipment', 2),
        ('Extension Cord - 50ft', 'SBR Equipment', 3),
        ('Extension Cord - 75ft', 'SBR Equipment', 0),
        ('Flir thermal Imaging Camera', 'SBR Equipment', 1),
        ('Furniture Polisher', 'SBR Equipment', 3),
        ('Garden Hose (Brass Shut Off)', 'SBR Equipment', 2),
        ('Garden Hose 5/8 x 25', 'SBR Equipment', 5),
        ('Garden Hose 5/8 x 50', 'SBR Equipment', 1),
        ('Garden Hose 75 ft', 'SBR Equipment', 4),
        ('Hand Clamp', 'SBR Equipment', 1),
        ('Heat gun', 'SBR Equipment', 1),
        ('Hedge clipper extended', 'SBR Equipment', 1),
        ('Hedge Trimmer STIHL (HANDHELD)', 'SBR Equipment', 2),
        ('Hole Digger', 'SBR Equipment', 1),
        ('Honda 2200W GAS Generator', 'SBR Equipment', 1),
        ('Jumper cables', 'SBR Equipment', 3),
        ('Ladder 24\' Extension', 'SBR Equipment', 1),
        ('Ladder 6\'', 'SBR Equipment', 2),
        ('LDC4000 Leak Detector combustion gas/ Freon', 'SBR Equipment', 1),
        ('Leaf Rake', 'SBR Equipment', 1),
        ('Light Bulb Removal Stick', 'SBR Equipment', 4),
        ('Lock Pick Set', 'SBR Equipment', 1),
        ('Moisture Meter', 'SBR Equipment', 1),
        ('Ortho GroundClear Vegetation Killer Concentrate 1 gal', 'SBR Equipment', 2),
        ('Pitchfork', 'SBR Equipment', 1),
        ('Plunger', 'SBR Equipment', 2),
        ('Pocket Garden Hose', 'SBR Equipment', 1),
        ('Pole Saw (STIHL Extension)', 'SBR Equipment', 2),
        ('Pool Leak Rake', 'SBR Equipment', 1),
        ('Pool Vacuum', 'SBR Equipment', 1),
        ('Pop up traffic cone', 'SBR Equipment', 4),
        ('Portable Utility Pump', 'SBR Equipment', 1),
        ('Pressure washer - (Honda)', 'SBR Equipment', 1),
        ('Pruner: 8" By- Pass', 'SBR Equipment', 1),
        ('Pump Transfer', 'SBR Equipment', 1),
        ('Racing Jack', 'SBR Equipment', 1),
        ('Ratchet Tie Down', 'SBR Equipment', 6),
        ('RIDGID Shop Vac', 'SBR Equipment', 2),
        ('Right Side Mirror for van 5', 'SBR Equipment', 5),
        ('Saw Blade', 'SBR Equipment', 2),
        ('Saw Circular', 'SBR Equipment', 1),
        ('SAW COPING PT HANDLE', 'SBR Equipment', 1),
        ('Shoulder Carry Straps', 'SBR Equipment', 3),
        ('Shovel (Flat head)', 'SBR Equipment', 5),
        ('Shovel Saw', 'SBR Equipment', 1),
        ('Sludge Blaster', 'SBR Equipment', 1),
        ('Spare tire', 'SBR Equipment', 5),
        ('Standard Cart Battery Hose', 'SBR Equipment', 2),
        ('Starter Handle', 'SBR Equipment', 1),
        ('Steamer (electrical)', 'SBR Equipment', 1),
        ('Swoosh CO2 Gun', 'SBR Equipment', 5),
        ('Tailgate Trim Cover', 'SBR Equipment', 1),
        ('Tire Iron - 4 way', 'SBR Equipment', 1),
        ('Toilet Auger (handheld)', 'SBR Equipment', 2),
        ('Tow Strap', 'SBR Equipment', 3),
        ('Trimmer (Dewalt)', 'SBR Equipment', 1),
        ('Trimmer, Hedge (STIHL Extension)', 'SBR Equipment', 1),
        ('Weed Eater STIHL', 'SBR Equipment', 1),
        ('Window Cleaner Fast Lock Hybrid Brush', 'SBR Equipment', 1),
        ('Wood Blade', 'SBR Equipment', 1),
        ('Wrench- Garbage Disposal', 'SBR Equipment', 1),
        ('3/8 IN Cable Clamp Connector', 'Technology Supplies', 6),
        ('Cable tie (200)', 'Technology Supplies', 2),
        ('Google Home WiFi Boost', 'Technology Supplies', 2),
        ('HDMI Cords', 'Technology Supplies', 2),
        ('Remote - Mediacom', 'Technology Supplies', 2),
        ('Remote - Roku', 'Technology Supplies', 4),
        ('Remote - Universal', 'Technology Supplies', 12),
        ('TV mount - hardware kit', 'Technology Supplies', 4),
        ('Universal Fan Remotes', 'Technology Supplies', 0),
        ('Milwaukee: OSC Wood Blade', 'Tools', 1),
        ('Denali Pure WF-4396508- S', 'Water Filters', 2),
        ('EDR1RXD1 Everydrop Water Filter', 'Water Filters', 2),
        ('EDR2RXD1 Everydrop Water Filter', 'Water Filters', 3),
        ('Everydrop by Whirlpool Ice Filter, F2WC9I1', 'Water Filters', 1),
        ('Filter Logic FL-RF46', 'Water Filters', 1),
        ('Fridge air filter AF-2-2', 'Water Filters', 2),
        ('GE MWF Replacement Refrigerator Water Filter', 'Water Filters', 2),
        ('GE RPWFE Water filter', 'Water Filters', 1),
        ('GE XWFE Refrigerator Water Filter', 'Water Filters', 2),
        ('LG LT1000P', 'Water Filters', 1),
        ('Mountain Flow MF841 water filter', 'Water Filters', 2),
        ('Project Source Af-3-2 Refrigerator Replacement Air Filter Fits F', 'Water Filters', 3),
        ('Project Source Refrigerator Water Filter W-2-2', 'Water Filters', 1),
        ('Pureline Replacement for MWF Water Filter', 'Water Filters', 2),
        ('RWF 1200A', 'Water Filters', 0),
        ('Ultra Clarity Water filter', 'Water Filters', 1),
        ('Water Drop WD-F07', 'Water Filters', 1),
        ('Waterdrop WDSF27', 'Water Filters', 2),
        ('WD-F02', 'Water Filters', 1),
        ]
        for name, category, qty in maintenance_parts_seed:
            cur.execute(
                "INSERT INTO supply_items (name,category,quantity,low_stock_threshold,unit,created_at) VALUES (%s,%s,%s,0,'units',%s) ON CONFLICT (name) DO NOTHING RETURNING id",
                (name, category, qty, now_central())
            )
            new_row = cur.fetchone()
            if new_row:
                qr = make_supply_qr(new_row[0])
                cur.execute("UPDATE supply_items SET qr_code=%s WHERE id=%s", (qr, new_row[0]))
        conn.commit()
    except Exception as e:
        print(f'Maintenance parts seed note: {e}')
        conn.rollback()
...
def _loaner_label_xy(row, col):
    page_w, page_h = letter
    x = _LOANER_LABEL_LEFT_MARGIN + col * _LOANER_LABEL_W
    y = page_h - _LOANER_LABEL_TOP_MARGIN - (row + 1) * _LOANER_LABEL_H
    return x, y
...
def loaners_qr_labels_pdf():
    ...
    if not items:
        return jsonify({'error': 'No loaner items found'}), 404

    sheets = [items[i:i + _LOANER_LABEL_PER_PAGE] for i in range(0, len(items), _LOANER_LABEL_PER_PAGE)]

    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=letter)
    for sheet in sheets:
        for i, item in enumerate(sheet):
            row, col = divmod(i, _LOANER_LABEL_COLS)
    ...
    buf.seek(0)
    return Response(buf.getvalue(), mimetype='application/pdf',
                     headers={'Content-Disposition': 'inline; filename="loaner-item-labels.pdf"'})
...
def pickup_bag(bag_id):
    ...
    # Once a bag is out the door, it can't be topped off anymore — if it
    # shipped short an item, that's just how it went out. "Packed Bag Short"
    # is for the window between packing and pickup where a top-off is still
    # possible; past that, an entry sitting there is just asking for
    # something that will never happen. Clear whatever's open for whichever
    # pack date staged this specific bag, same lookup get_staged_bags already
    # uses to find a bag's pack_date from its staged_bag_ids record.
    cur.execute("""SELECT pls.pack_date FROM pack_list_status pls
                   WHERE pls.cleaner_id=%s
                     AND %s = ANY(string_to_array(pls.staged_bag_ids, ','))
                   ORDER BY pls.pack_date DESC LIMIT 1""", (cleaner['id'], bag_id.upper()))
    pd_row = cur.fetchone()
    if pd_row:
        cur.execute(
            "UPDATE pack_bag_shortages SET resolved=1 WHERE address=%s AND pack_date=%s AND resolved=0",
            (bag['home_name'].lower().strip(), pd_row['pack_date'])
        )
    conn.commit(); cur.close(); conn.close()
...
@app.route('/api/supplies/qr-sheet', methods=['GET'])
def supplies_qr_sheet():
    """Generate printable QR codes for maintenance supply items — one per
    item (not per unit), meant for the box/bin holding that part. Scanning it
    is meant to jump straight to that item so a count can be adjusted."""
    conn = get_db(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT id, name, category, qr_code FROM supply_items ORDER BY category, name")
    rows = cur.fetchall(); cur.close(); conn.close()
    result = [{'id': r['id'], 'name': r['name'], 'category': r['category'],
               'qr_code': r['qr_code'] or make_supply_qr(r['id'])} for r in rows]
    return jsonify(result)
...
def compute_pack_schedule(window_start, window_end):
    ...
    # blocks, owner stays Breezeway doesn't feed into the reservation pull —
    # still needs a bag packed. If it's a clean in Breezeway, it needs to be
    # in the app. Derives pack_date straight from the task's own scheduled
    # date, same principle as the long-standing Bay House fallback below,
    # generalized to any address instead of one hardcoded name.
    #
    # Hard floor at today (fallback_start_dt), independent of whatever window
    # the caller passed in: this function is also called with wide historical
    # lookbacks (e.g. the 30-day "missed" check), and SBR wasn't tracking
    # every pack in pack_list_status before go-live — there's no reliable way
    # to tell "genuinely missed" apart from "handled off-system" for anything
    # before now. Rather than guess, nothing before today is ever considered
    # here, no matter which screen or lookback is asking.
    #
    # Skipped deliberately:
    #   - cleans already consumed as a real checkout match (matched_keys)
    #   - cleans already claimed as the "nearest" candidate for a
    #     date_mismatch issue (flagged_keys) — an actual person should look
    #     at those, not have them silently auto-scheduled out from under
    #     the review queue
    #   - anything already packed (pack_list_status) or already handled via
    #     a manual Emergency Add (pack_emergency_adds) — same principle used
    #     just above: whether it was actually handled, not how old it is
    #   - TASK_ONLY_PROPERTIES addresses, handled by their own dedicated
    #     block right below with proper address-name translation
    fallback_start_dt = max(window_start_dt, today_dt)
